...
export function buildPassLedger(
  ...
  const deductions = db.prepare(`
    ...
    WHERE sa.pass_id = ?
  `).all(passId) as RawDeduction[];

  const topups = db.prepare(`
    SELECT created_at AS ts, sessions FROM pass_topups WHERE pass_id = ?
  `).all(passId) as RawTopup[];

  const manualDeductions = db.prepare(`
    SELECT created_at AS ts, sessions, note FROM pass_manual_deductions WHERE pass_id = ?
  `).all(passId) as RawManualDeduction[];
